...
| 항목 | 내용 |
|:---:|:---|
| **제목** | 오늘은 잠들 수 있을까? - 뚝★배기 에디션 |
| **장르** | 3D 타임어택 액션 |
| **플랫폼** | PC (Unity) |
| **인원** | 1인 (Single Player) |
| **플레이 타임** | 1회차(7일) 기준 약 30~40분 |

### 2. 핵심 컨셉 (Core Concept)
**"잠들기 힘든 소음 아파트에서 7일간 살아남아라!"**
매일 밤 쏟아지는 이웃들의 방해 공작 속에서, 피로도가 **100**에 도달하지 않도록 관리하며 일주일(7일)을 버티는 것이 목표.
> **"플레이어는 숙면에 방해가 되는 요소들을 제거하여, 피로도가 100이 되지 않게 관리하며 버텨야 한다."**

### 3. 핵심 재미 & 기획 의도

**🤩 핵심 감정 (Core Emotions)**
* **현실 해소의 쾌감:** 현실에서는 참아야 할 층간소음 상황을 내 손으로 직접 끝장내 버릴 때의 속 시원함.
* **피지컬 액션의 스릴:** 쏟아지는 소음과 사건을 제한 시간 내에 몸으로 뛰며 해결하는 스릴 (두더지 잡기 게임과 유사한 재미).

**💡 기획 의도 (Design Intent)**
* **난장판을 완벽하게 정리하는 쾌감**
    * 그 어떤 것도 거슬리지 않는 깔끔한 상태를 만들고자 하는 강박적인 욕구 충족.
* **통제력 회복의 즐거움**
    * 통제 가능하고 극복할 수 있는 스트레스를 해결하며 얻을 수 있는 전능감.
* **공감대가 만드는 몰입**
    * 보편적인 고통(이웃의 소음으로 인한 괴로움)을 통해 플레이어들의 자연스러운 몰입 유도.

### 4. 승리 및 패배 조건 (Conditions)
게임의 엔딩은 최종 피로도와 생존 여부에 따라 2가지로 나뉜다.

| 구분 | 엔딩 명 | 상세 조건 |
|:---:|:---:|:---|
| **승리** | **Normal Ending** | 7일 생존 성공 (피로도 30 초과 ~ 100 미만) |
| **패배** | **Bad Ending** | 플레이 도중 피로도 **100 도달** (기절 → 지각 → 해고 💀) |

### 5. 게임 루프 & 시간 시스템 (Game Loop & Time Scale)
플레이어는 악덕 기업(Black Company)의 직원으로, 주말 없이 **7일** 동안 매일 밤 출근 전쟁을 치러야 한다.

* **하루 구성 (Daily Routine):** `00:00 ~ 06:00` (총 6시간)
    * 퇴근 후 집에 도착한 시점부터, 다음 날 출근 준비 직전까지의 시간.
* **시간 비율 (Time Scale):**
    * 현실 시간 **1초** ≒ 게임 시간 **1분 30초**
    * 현실 시간 **4분** = 게임 시간 **6시간 (하루)**
* **핵심 메카닉 (Core Mechanic):**
    > **"문제를 해결할 것인가, 잠을 잘 것인가?"**
    * 플레이어에게 주어진 물리적 시간은 단 **4분**.
    * 이 시간 안에 소음원(방해 요소)을 찾아 해결하고 침대로 돌아와야 한다.
    * **Time Trade-off:** 소음을 완벽하게 해결하려 시간을 쓸수록 **수면 시간(남은 시간)**이 줄어든다.
    * 침대에 눕는 순간 남은 시간이 곧 수면 시간이 되어 피로도가 회복되므로, **빠른 해결과 휴식 사이의 효율적인 판단**이 요구다.
